...
| Code host | **GitHub App + REST/GraphQL + webhooks**; adapter interface. | GitLab first (a GitLab adapter behind the same interface landed in 3.7: MRs as gates, commit statuses as checks). | GitHub-only initially. | Playbook's reference integrations (claude-code-action, Code Review, Claude Security GitHub App). |
...
| CI | **GitHub Actions workflows generated by `sdlc init`** (evals on `CLAUDE.md`/`.claude/**`, per-change run, validate on PR). | GitLab CI: `sdlc init --code-host gitlab` writes `.gitlab-ci.yml` with the same jobs (3.7). | — | Playbook YAML is the template. |
