...
- [ ] 0.5 Transitions as write-plans: `createChange`, `accept`, `sendBack`, `loop` (cycle+1 + archive + INC draft case), `confirmTasks`, `confirmRepro`. Return `{files, events, commitMessage}`; no I/O.
- [ ] 0.6 Validation engine: blocking rules from blueprint §11.1 that need no adapters (schema, completeness, SHA chaining, gate ownership + human actor, risk/kind immutability, duplicate ids, active-case-has-checks, linked-mode fields).
