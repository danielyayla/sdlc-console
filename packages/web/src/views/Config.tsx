...
export function Config({ snapshot, onDismissProposal, prompt = (t) => window.prompt(t) }: ConfigProps) {
  ...
  const threshold = snapshot.config.evals.threshold;
  const passPct = latest ? Math.round(latest.passRate * 100) : null;
  const suiteMin = snapshot.config.thresholds.suiteMinSize;
  const underSized = active.length < suiteMin;
  ...
  return (
    <div className="config">
      <div className={`banner${underSized || (passPct !== null && passPct < threshold * 100) ? "" : " green"}`}>
        <span className={`chip ${underSized ? "amber" : "green"}`}>suite {active.length}{underSized ? ` · under-sized (< ${suiteMin})` : ""}</span>{" "}
        <span className={`chip ${passPct === null ? "gray" : passPct >= threshold * 100 ? "green" : "amber"}`}>pass {passPct === null ? "n/a" : `${passPct}%`} · threshold {Math.round(threshold * 100)}%</span>{" "}
        <span className="chip gray">{snapshot.config.evals.mode}{snapshot.config.evals.mode === "scheduled" ? " · config PRs not gated" : ""}</span>{" "}
        <span className="chip gray">budget {snapshot.config.evals.budget === null ? "n/a" : `${latest?.cost ?? 0} / ${snapshot.config.evals.budget}`}</span>{" "}
        <button className="btn" disabled title="Run suite needs the CI adapter (Phase 2)">Run suite</button>
      </div>
      ...
      <section className="panel">
        ...
        <div className="strip">{runs.slice(-30).map((r) => <span key={r.id} className={`dot ${r.verdict === "pass" ? "green" : r.verdict === "fail" ? "amber" : "inactive"}`} title={`${r.id} · ${r.trigger} · ${Math.round(r.passRate * 100)}% · ${r.configRef.model}`} />)}{runs.length === 0 ? <span className="muted">no runs yet</span> : null}</div>
